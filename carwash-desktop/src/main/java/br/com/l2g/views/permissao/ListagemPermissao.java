 ...
 */
public class ListagemPermissao extends javax.swing.JInternalFrame {

    /**
     * Creates new form ListagemUsuario
     */
    private static final String URL_BASE = Environment.DEV.url();
    private static final String URL_GRUPOPERMISSAO = URL_BASE + "grupopermissao";
    private final Usuario usuarioTelaPrincipal;

    public ListagemPermissao(Usuario usuarioTelaPrincipal) {
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setTitle("Listagem de Permissões");
    ...
    private void CarregaTabela() {
        try {
            ...
            HttpGet get = new HttpGet(URL_GRUPOPERMISSAO);
            String resposta = Util.enviaRequest(get);
            List<GrupoPermissao> clienteList = Arrays.asList(Util.jsonToObject(resposta, GrupoPermissao[].class));
            for (GrupoPermissao c : clienteList) {
                objects[0] = c.getCodigoGrupoPermissao();
                objects[1] = c.getDescricaoGrupoPermissao();
            ...
            Logger.getLogger(ListagemPermissao.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(null, ex.getMessage(), "ERRO", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void IncluirGrupoPermissao() {
        CadastroPermissao view = new CadastroPermissao(null, true, "incluir");
        view.setVisible(true);
        CarregaTabela();
    }

    private void AlterarGrupoPermissao() {
        CadastroPermissao view = new CadastroPermissao(null, true, "editar");
        view.enviarCodigoSelecionado(jTable1.getModel().getValueAt(jTable1.getSelectedRow(), 0).toString());
        view.setVisible(true);
        CarregaTabela();
    }

    private void DeletarGrupoPermissao() {
        try {
            ...
            if (opcao == 0 || opcao == -1) {
                String url_id = URL_GRUPOPERMISSAO + "/" + jTable1.getModel().getValueAt(jTable1.getSelectedRow(), 0).toString();
